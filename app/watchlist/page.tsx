          ...
          {watched.map((item, i) => (
            <div key={item.id} style={{
              ...
              <div>
                ...
                <span style={{ fontSize: '11px', color: 'var(--text-muted)', marginLeft: '8px' }}>{item.name}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                <span style={{ fontSize: '13px', fontWeight: 500 }}>{formatPrice(item.price)}</span>
                <span style={{ fontSize: '12px', color: changeColor(item.change24h), fontWeight: 500, minWidth: '60px', textAlign: 'right' }}>{formatChange(item.change24h)}</span>
                <button onClick={() => remove(item.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>
                  <Trash2 size={13} />
                </button>
              </div>
            </div>
