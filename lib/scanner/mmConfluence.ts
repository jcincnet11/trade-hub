...
export function scoreSetup(input: ScoreInput): ScoreOutput {
  ...
  const pattern =
    patterns.find((p) => (direction === 'long' ? p.type === 'bullish' : p.type === 'bearish')) ?? null
...
export function checkInvalidation({ direction, candles, pattern }: InvalidationInput): string | null {
  const n = candles.length
  if (n < 20) return null
  const last = candles[n - 1]
  const last20 = candles.slice(-20)
